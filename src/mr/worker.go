...
func Worker(mapf func(string, string) []KeyValue,
	...
	for {
		...
		task := reply.TaskStatus
		switch task {
		case "map":
			doMapTask(&reply, mapf)
			call("Coordinator.TaskDone", &TaskProgressArgs{
				TaskStatus: "map",
				TaskId:     reply.TaskId,
			}, &TaskProgressReply{})
		case "reduce":
...
func doMapTask(task *GetTaskReply,
	...
	// write into temporary file (for reduce to read)
	for i := 0; i < nReduce; i++ {
...
func doReduceTask(task *GetTaskReply,
	reducef func(string, []string) string) {
	// read all related file
